...
clear
tls="$(cat ~/log-install.txt | grep -w "Grpc Vmess TLS" | cut -d: -f2|sed 's/ //g')"
vl="$(cat ~/log-install.txt | grep -w "Grpc Vless" | cut -d: -f2|sed 's/ //g')"
echo -e "======================================"
echo -e ""
echo -e "[1]. Change Port Vmess TLS $tls"
echo -e "[2]. Change Port Vmess None TLS $vl"
echo -e "[3]. Exit"
echo -e ""
echo -e "======================================"
echo -e ""
...
case $prot in
1)
read -p "New Port Grpc Vmess TLS : " tls1
if [ -z $tls1 ]; then
...
sed -i "s/   - Grpc Vless             : $tls/   - Grpc Vless             : $tls1/g" /root/log-install.txt
...
read -p "New Port Grpc Vless : " none1
if [ -z $none1 ]; then
echo "Please Input Port"
exit 0
fi
...
sed -i "s/   - Grpc Vless          : $none/   - Grpc Vless        : $none1/g" /root/log-install.txt
